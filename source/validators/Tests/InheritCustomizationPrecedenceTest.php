...
/**
 * Verifies that components with token-backed component settings resolve
 * component overrides before --inherit-* fallbacks.
 */
class InheritCustomizationPrecedenceTest extends TestCase
{
    /**
    ...
    public function testTokenBackedComponentSettingsOutrankInheritFallbacks(string $filePath): void
    {
        ...
        $validator = new InheritCustomizationPrecedenceValidator();
        $result = $validator->validate($filePath);

        $this->assertTrue(
            $result->isValid(),
            sprintf(
                "Invalid inherit precedence pattern in %s:\n%s",
