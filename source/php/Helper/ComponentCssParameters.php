        ...
        foreach ($componentTokens as $token) {
            ...
            if ($token === 'border-radius') {
                self::appendRow(
                    $rows,
                    $seenVariables,
                    self::toLocalizedVariable($componentPrefix, 'corner-shape'),
                    'var(--corner-shape)',
                    $settingsByToken['corner-shape'] ?? null
                );
            }

            if ($token === 'shadow') {
                foreach (['shadow-color', 'shadow-color-opacity', 'shadow-amount'] as $shadowToken) {
                    self::appendRow(
                        $rows,
                        $seenVariables,
                        self::toLocalizedVariable($componentPrefix, $shadowToken),
                        'var(--' . $shadowToken . ')',
                        $settingsByToken[$shadowToken] ?? null
                    );
                }
            }

            if (str_starts_with($token, 'color') && !str_ends_with($token, '-contrast')) {
                $contrastToken = $token . '-contrast';
                self::appendRow(
                    $rows,
                    $seenVariables,
                    self::toLocalizedVariable($componentPrefix, $contrastToken),
                    'var(--' . $contrastToken . ')',
                    $settingsByToken[$contrastToken] ?? null
                );

                if (str_contains($token, 'surface') || str_contains($token, 'background')) {
                    $mutedToken = $token . '-contrast-muted';
                    self::appendRow(
                        $rows,
                        $seenVariables,
                        self::toLocalizedVariable($componentPrefix, $mutedToken),
                        'var(--' . $mutedToken . ')',
                        $settingsByToken[$mutedToken] ?? null
                    );
                }

                $localizedTokenVariable = self::toLocalizedVariable($componentPrefix, $token);
                $localizedContrastVariable = self::toLocalizedVariable($componentPrefix, $contrastToken);
                $localizedMutedVariable = self::toLocalizedVariable($componentPrefix, $token . '-contrast-muted');

                self::appendRow(
                    $rows,
                    $seenVariables,
                    self::toLocalizedVariable($componentPrefix, $token . '-border'),
                    'color-mix(in srgb, var(' . $localizedContrastVariable . ') 10%, var(' . $localizedTokenVariable . '))',
                    [
                        'type' => 'computed',
                        'description' => 'Derived border color for this component token.',
                    ]
                );
                ...
                self::appendRow(
                    $rows,
                    $seenVariables,
                    self::toLocalizedVariable($componentPrefix, $token . '-alt'),
                    'color-mix(in srgb, var(' . $localizedMutedVariable . ', var(' . $localizedContrastVariable . ')) 4%, var(' . $localizedTokenVariable . '))',
                    [
                        'type' => 'computed',
                        'description' => 'Derived alt color for this component token.',
                    ]
                );
            }
        }
