import { bindManualCustomizationInitTrigger } from './bindManualCustomizationInitTrigger';

describe('bindManualCustomizationInitTrigger', () => {
	beforeEach(() => {
	...
	it('binds initialization to the nested FAB toggle trigger instead of the whole container', () => {
		...
		const unbind = bindManualCustomizationInitTrigger(initializer);
	...
	it('falls back to initializing when no manual trigger exists on the page', () => {
		...
		bindManualCustomizationInitTrigger(initializer);
