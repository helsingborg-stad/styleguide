import { GENERAL_SCOPE_KEY, NON_CUSTOMIZABLE_COMPONENTS } from '../state/runtimeConstants';
...
function hasLocalScopeOverrideForElement(
	overrides: ScopedComponentOverrides,
...
function getElementsForContext(
	elementsByComponent: Map<string, HTMLElement[]>,
	componentName: string,
	scopeKey: string,
): HTMLElement[] {
...
export function applyPersistedTokenOverrides(overrides: Record<string, string>): void {
	for (const [variable, value] of Object.entries(overrides)) {
		document.documentElement.style.setProperty(variable, value);
	}
}

export function clearPersistedTokenOverrides(overrides: Record<string, string>): void {
	for (const variable of Object.keys(overrides)) {
		document.documentElement.style.removeProperty(variable);
	}
}

export function applyPersistedComponentOverrides(overrides: ScopedComponentOverrides): void {
			...
			for (const [variable, value] of Object.entries(componentOverrides)) {
				...
				if (scopeKey === GENERAL_SCOPE_KEY) {
					elements = elements.filter(
						(element) => !hasLocalScopeOverrideForElement(overrides, componentName, variable, element),
					);
				}
...
export function clearPersistedComponentOverrides(overrides: ScopedComponentOverrides): void {
	const elementsByComponent = getElementsByComponent();
