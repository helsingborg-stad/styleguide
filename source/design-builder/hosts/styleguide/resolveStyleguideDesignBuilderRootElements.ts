...
import { resolveStyleguideCustomizeInitMode } from './resolveStyleguideCustomizeInitMode';
...
function mergeCustomizeConfig(rootElement: DesignBuilderRootElement): void {
	...
	existingConfig.initMode = resolveStyleguideCustomizeInitMode();
...
function hydrateComponentPayload(rootElement: DesignBuilderRootElement): void {
	const hasWindowPayload = window.styleguideCustomizeData !== undefined && window.styleguideDesignTokenLibrary !== undefined;
	if (!hasWindowPayload) {
		return;
	}
...
function createComponentRootElementFromPayload(): DesignBuilderRootElement | null {
	...
	componentRootElement.setAttribute('config', JSON.stringify({ initMode: resolveStyleguideCustomizeInitMode() }));
