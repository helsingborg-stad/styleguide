...
export function parseDesignBuilderRootConfiguration(input: ParseRootConfigurationInput): DesignBuilderRootConfiguration {
	const {
		hostElement,
		propertyMode,
		propertyConfig,
...
const resolvedMode =
propertyMode ?? modeFromAttribute ?? modeFromConfig ?? DESIGN_BUILDER_MODE_FULL_PAGE;
...
...(propertyConfig ?? {}),
	...
	const tokenLibraryData =
		propertyTokenLibraryData ?? parseJsonUnknown(hostElement.getAttribute('token-library'));
