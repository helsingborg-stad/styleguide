...
const _Chat = class _Chat {
  ...
  addMessage(messageContent, isReply = false, shouldPersist = true, id) {
...
const _Message = class _Message {
  constructor(isReply, message, messageContainer, id, content) {
...
const _Tooltip = class _Tooltip {
  init() {
    const triggers = Array.from(document.querySelectorAll(TooltipSetup.TriggerSelector));
    if (triggers.length === 0) {
      return;
    }
