...
function sanitizeNode(node) {
  const forbiddenTags = /* @__PURE__ */ new Set(["script", "style", "iframe", "object", "embed", "form", "link", "meta", "base"]);
  ...
  Array.from(node.children).forEach((child2) => {
    ...
    Array.from(child2.attributes).forEach((attribute) => {
      if (attribute.name.startsWith("on") || unsafeUrlAttributes.has(attribute.name.toLowerCase()) && unsafeUrlPattern.test(attribute.value.trim())) {
...
const _Chat = class _Chat {
  constructor(messageArea, input, messageFactory, messageStore) {
...
const _MessageStore = class _MessageStore {
...
__name(_MessageStore, "MessageStore");
let MessageStore = _MessageStore;
function init() {
  document.addEventListener("DOMContentLoaded", () => {
