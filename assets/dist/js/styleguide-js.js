...
(() => {
  addEventListener("DOMContentLoaded", init$9);
})();
...
const _Chat = class _Chat {
  constructor(container, messageArea, input, messageFactory, messageStore, clear, messageRenderer, pendingMessageManager) {
    this.container = container;
  ...
  runUserMessageCallback(message) {
  ...
  addMessage(messageContent, isReply = false, id) {
  ...
  createMessage(messageContent, isReply, shouldPersist, id) {
...
const _Message = class _Message {
  constructor(isReply, message, id, content) {
...
const _MessageFactory = class _MessageFactory {
  ...
  userMessageTemplate;
  create(message, isReply = false, id) {
...
const _LocalStorage = class _LocalStorage {
  ...
  isPersistentMessage(message) {
    return typeof message === "object" && message !== null && typeof message.id === "string" && typeof message.content === "string" && typeof message.isReply === "boolean";
  }
};
...
const _BasicStorage = class _BasicStorage {
  ...
  persistentMessageIds = /* @__PURE__ */ new Set();
  getAll() {
  ...
  getSavedMessages() {
    return this.getAll().filter((message) => this.persistentMessageIds.has(message.getId())).map((message) => ({
      id: message.getId(),
...
const _Load = class _Load {
  constructor(chat, storage, container) {
    this.chat = chat;
    this.storage = storage;
    this.container = container;
  }
...
function init$7() {
  document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll("[data-js-chat]").forEach((chatContainer) => {
      ...
      const messageArea = chatContainer.querySelector("[data-js-message-area]");
      ...
      const messageRenderer = new MessageRenderer(messageArea, chatContainer);
      ...
      new Load(chat, store, chatContainer).load();
      chat.init();
