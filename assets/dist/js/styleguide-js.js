...
(() => {
  addEventListener("DOMContentLoaded", init$c);
})();
...
function enableHashLinkExpansion() {
  document.addEventListener("click", (event) => {
    ...
    const link = event.target.closest('.c-nav__item-wrapper > a[href="#"]');
    const itemWrapper = link?.closest(".c-nav__item-wrapper");
    const toggle2 = itemWrapper?.querySelector(":scope > .c-nav__toggle");
    if (!link?.closest(".c-nav") || !toggle2) {
      return;
    }
    event.preventDefault();
    event.stopPropagation();
