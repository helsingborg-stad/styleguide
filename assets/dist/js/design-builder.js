...
const _DesignBuilderCustomElement = class _DesignBuilderCustomElement extends HTMLElement {
  ...
  currentOverrideState = createEmptyOverrideState();
...
__publicField(_DesignBuilderCustomElement, "observedAttributes", ["token-data", "token-library", "component-data", "override-state"]);
...
const _ComponentCustomizerRuntime = class _ComponentCustomizerRuntime {
  ...
  renderPresetChipTemplate(name, isActive) {
    return b`
			<button
				type="button"
			...
			>
				...
				<span
					...
					title=${`Delete "${name}"`}
					@click=${(event) => this.handleDeletePresetClick(event, name)}
				>
					&times;
				</span>
			</button>
  ...
  loadPreset(name) {
    ...
    clearTokenOverridesFromRootDocument(this.hostElement?.overrideState.token ?? {});
    applyTokenOverridesToRootDocument(presetOverrides.token);
  ...
  handleDeletePresetClick = /* @__PURE__ */ __name((event, name) => {
    event.stopPropagation();
