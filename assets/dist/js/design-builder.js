...
function positionTooltip(anchor, tooltipElement) {
  ...
  const top = fitsBelow ? preferredBottomTop : Math.max(TOOLTIP_VIEWPORT_MARGIN, anchorRect.top - tooltipRect.height - TOOLTIP_OFFSET);
...
function createHoverTipController(root) {
  const resetHoverTip = /* @__PURE__ */ __name(() => {
    const variableElement = root.querySelector("[data-hover-tip-variable]");
    const descriptionElement = root.querySelector("[data-hover-tip-description]");
  ...
  const updateHoverTip = /* @__PURE__ */ __name((target) => {
    const variableElement = root.querySelector("[data-hover-tip-variable]");
    const descriptionElement = root.querySelector("[data-hover-tip-description]");
    if (!variableElement || !descriptionElement) {
      return;
    ...
    const tipTarget = target.closest(HOVER_TIP_TARGET_SELECTOR);
    if (!tipTarget) {
      resetHoverTip();
      return;
    }
  ...
  const handlePointerOver = /* @__PURE__ */ __name((event) => {
    updateHoverTip(event.target);
  }, "handlePointerOver");
  const handlePointerOut = /* @__PURE__ */ __name((event) => {
    const tipTarget = event.target instanceof Element ? event.target.closest(HOVER_TIP_TARGET_SELECTOR) : null;
    if (!tipTarget) {
      return;
    }
    const relatedTarget = event.relatedTarget;
    const relatedTipTarget = relatedTarget instanceof Element ? relatedTarget.closest(HOVER_TIP_TARGET_SELECTOR) : null;
...
__name(createHoverTipController, "createHoverTipController");
