...
const _ComponentCustomizationTool = class _ComponentCustomizationTool {
  ...
  setActiveTarget(componentName, preferredElement) {
    ...
    for (const category of categories) {
      ...
      for (const setting of category.settings) {
        ...
        const control = createControl(setting, currentValue, (variable, value) => {
          this.handleChange(this.activeComponent, variable, value, setting.default);
        });
  ...
  handleChange(componentName, variable, value, defaultValue) {
  ...
  applyVariable(componentName, variable, value) {
    const elements = this.elementsByComponent.get(componentName) || [];
    for (const element of elements) {
      element.style.setProperty(variable, value);
    }
  }
  removeVariable(componentName, variable) {
    const elements = this.elementsByComponent.get(componentName) || [];
    for (const element of elements) {
