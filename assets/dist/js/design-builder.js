...
const _ComponentCustomizationTool = class _ComponentCustomizationTool {
  ...
  getAvailableScopeKeys(componentName) {
    ...
    const specificScopes = Array.from(availableScopes).filter((scopeKey) => scopeKey !== GENERAL_SCOPE_KEY && scopeKey !== GLOBAL_SCOPE_KEY).sort((a, b) => a.localeCompare(b));
...
function resolveCustomizeInitMode() {
  const rawMode = typeof window.styleguideCustomizeInitMode === "string" ? window.styleguideCustomizeInitMode.toLowerCase().trim() : "";
  if (rawMode === CUSTOMIZE_INIT_MODE_MANUAL) {
    return CUSTOMIZE_INIT_MODE_MANUAL;
  }
  return CUSTOMIZE_INIT_MODE_ONLOAD;
}
__name(resolveCustomizeInitMode, "resolveCustomizeInitMode");
...
function initializeDesignBuilderContainer() {
...
__name(initializeDesignBuilderContainer, "initializeDesignBuilderContainer");
async function initializeComponentCustomizationTool() {
...
__name(initializeComponentCustomizationTool, "initializeComponentCustomizationTool");
function openComponentCustomizationPanel() {
  const panelRoot = document.querySelector(".db-component-tool");
  if (!panelRoot) {
    return;
  }
  panelRoot.classList.add("db-component-tool--open");
}
__name(openComponentCustomizationPanel, "openComponentCustomizationPanel");
function bindManualCustomizationInitTrigger() {
  const triggers = document.querySelectorAll(CUSTOMIZE_MANUAL_TRIGGER_SELECTOR);
...
__name(bindManualCustomizationInitTrigger, "bindManualCustomizationInitTrigger");
async function init() {
