...
const _ComponentCustomizerRuntime = class _ComponentCustomizerRuntime {
  ...
  renderControl(setting) {
    ...
    return createDesignBuilderControl(setting, currentValue, (variable, value, extraValues, options) => {
      this.handleChange(this.activeComponent, this.activeScopeKey, variable, value, setting.default, setting.linkedDefaults, extraValues, options);
    });
  }
  ...
  matchesVisibilityCondition(targetElement, condition) {
    if (!condition) {
      return true;
    }
  ...
  isComponentSettingVisible(setting, targetElements) {
  ...
  buildCategoriesForComponent(componentName) {
    ...
    for (const category of componentSettings) {
      const matchedSettings = category.settings.filter((setting) => this.isComponentSettingVisible(setting, targetElements)).map((setting) => this.resolveComponentSetting(componentName, availableTokenNames, hiddenTokenNames, setting)).filter((setting) => setting !== null);
      this.appendCategory(categories, {
        id: category.id,
        label: category.label,
        description: category.description,
