...
function formatScaleStep(fontSizeScaleStep) {
	return Number.isInteger(fontSizeScaleStep) ? String(fontSizeScaleStep) : String(fontSizeScaleStep);
}

function buildFontSizeScaleValue(fontSizeScaleStep) {
	if (fontSizeScaleStep === 0) {
		return 'var(--base-font-size)';
	}

	return `calc(var(--base-font-size) * pow(var(--font-size-scale-ratio), ${formatScaleStep(fontSizeScaleStep)}))`;
}
...
function formatValue(setting) {
	if (isFiniteNumber(setting.fontSizeScaleStep)) {
		return buildFontSizeScaleValue(setting.fontSizeScaleStep);
	}
